...
\begin{frame}
  ...
  \begin{center}
    \includegraphics[height=0.7\textheight]{slides/kernel-driver-development-memory/memory-mapping-64b.pdf}
  \end{center}
  See also the documentation in \kdochtml{arch/x86/x86_64/mm}
\end{frame}
...
\begin{frame}
  ...
  \item \ksym{GFP_DMA}
    \begin{itemize}
    \item Allocates memory in an area of the physical memory usable for
      DMA transfers.
    \end{itemize}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item \kconfig{CONFIG_SLAB}: legacy but now deprecated (and removed in Linux 6.8)
