...
\begin{frame}[fragile]
  \frametitle{Kconfig}
  Kernel configuration options are defined in \code{Kconfig} files:
  \begin{itemize}
  \item There are approximately 1600 Kconfig files throughout the tree (as of v6.1)
  \item \code{Kconfig} uses a dedicated language, which is documented in
        the kernel's \href{https://www.kernel.org/doc/html/next/kbuild/kconfig-language.html}{Kconfig Language}.
  \end{itemize}

  \medskip
  Example (from \kfile{drivers/tty/serial/8250/Kconfig}):
  \begin{block}{}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Equivalent to \code{make all}
...
\begin{frame}[fragile]
  \frametitle{Kernel compilation: make targets}
  Instead of building everything, you can build a specific component:
  \begin{itemize}
  \item \code{make Image}: uncompressed kernel image
  \item \code{make zImage}: compressed ARM kernel
  \item \code{make dtbs}: Device Tree Blobs for enabled boards
  \item \code{make target.dtb}: a specific Device Tree Blob located in \code{arch/<arch>/boot/dts/target.dtb}
  \item \code{make dir/file.ko}: a specific module
  \end{itemize}

  \medskip
  Refer to \code{make help} for more options.
\end{frame}
...
\begin{frame}
  ...
  \item Therefore making the kernel image (\code{zImage} + \code{target.dtb}) available to the target is
