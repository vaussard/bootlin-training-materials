...
\begin{tabular}{l|c|c|c|c|}
                           & \textbf{softirq} & \textbf{tasklet} & \textbf{threaded IRQ} & \textbf{workqueue}    \\ 
\hline
\textbf{Context}           & Interrupt        & Interrupt        & Process               & Process               \\ 
\hline
\textbf{Can sleep}         & NO!              & NO!              & Yes                   & Yes (care if shared)  \\ 
\hline
\textbf{Dedicated process} & -                & -                & Dedicated             & Dedicated or shared   \\ 
\hline
\textbf{Schedule latency}  & "immediate"      & "immediate"      & Scheduled             & Scheduled             \\
\hline
\end{tabular}
}
\end{table}
...
\begin{frame}
  ...
  \item Use a \kstruct{task_struct} like userspace threads, but without a userspace address space
        and run solely in kernel mode
  \item Creation:
    \begin{itemize}
    \item Use \kfunc{kthread_create} followed by \kfunc{wake_up_process}
    \item Or simpler: \kfunc{kthread_run}
    \end{itemize}
  \item Can be bound to a specific CPU using \kfunc{kthread_bind}
        (or directly using \kfunc{kthread_run_on_cpu})
  \item \kfunc{kthread_stop} and \kfunc{kthread_should_stop} similar to pco-synchro ;-)
  \item To exit the kthread: \code{return;} from its main function
  \end{itemize}
\end{frame}
